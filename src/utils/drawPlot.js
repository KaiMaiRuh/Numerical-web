...
/**
 * drawPlot(canvas, func, xl, xr, iterations, options)
 * options: { width, height, bg, curveColor, axesColor, pointColor, lastPointColor }
 */
export function drawPlot(canvas, func, xl = -5, xr = 5, iterations = [], options = {}) {
  if (!canvas) return;
  const {
    width = 800,
    height = 320,
  ...
  iterations.forEach((it) => {
    ...
    ctx.beginPath(); ctx.arc(px, py, 3, 0, 2 * Math.PI); ctx.fillStyle = pointColor; ctx.fill();
  });
  ...
  if (iterations.length) {
    ...
    ctx.beginPath(); ctx.arc(px, py, 6, 0, 2 * Math.PI); ctx.fillStyle = lastPointColor; ctx.fill();
  }
}
