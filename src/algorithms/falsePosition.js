export function falsePosition(func, a, b, tol = 1e-6, maxIter = 50) {
  try {
    ...
    const iterations = [];
    let x1 = null, prev = null;
    for (let i = 0; i <= maxIter; i++) {
      x1 = (a * fb - b * fa) / (fb - fa);
      const fx1 = func(x1);
      if (!isFinite(fx1)) return { error: "f(x1) คำนวณไม่ได้", iterations, converged: false };

      const err = prev === null ? null : Math.abs((x1 - prev) / (Math.abs(x1) > 1e-12 ? Math.abs(x1) : 1));
