...
import * as NewtonRaphsonService from "../services/NewtonRaphsonService";
import useProblems from "../hooks/useProblems";
import PageHeader from "../components/PageHeader";
import SavedProblems from "../components/SavedProblems";
import { makeFunc, formatNum } from "../utils/math";
import { newtonRaphson } from "../algorithms/newtonRaphson";
...
export default function NewtonRaphson() {
  ...
  const { problems, removingIds, refresh, saveProblem, deleteProblem } =
    useProblems(NewtonRaphsonService);
  ...
  const handleRun = () => {
    ...
    if (!Number.isInteger(maxVal) || maxVal <= 0) {
      setStatus("สถานะ: Max Iteration ต้องเป็นจำนวนเต็มบวก");
      return;
    }
  ...
  const handleSaveProblem = async () => {
    ...
    try {
      await saveProblem({ expr, x0, tol, maxIter, method: "newton_raphson" });
      alert("บันทึกแล้ว!");
    } catch (e) {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <PageHeader title="Newton-Raphson Method" subtitle="ตาราง + กราฟ แสดงการทำงาน (เวอร์ชันใหม่)" />
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-800 rounded-lg p-4 shadow fade-in-delay2">
          ...
          <div className="text-xs text-gray-400 mt-2">
            เส้นสีฟ้า = f(x), จุดส้ม = ค่าที่ได้ในแต่ละ iteration, จุดสุดท้าย = ค่ารากสุดท้าย
          </div>
        </div>
      </div>
